...
oz_BitBuffer oz_serializeCharData(oz_CharData* cd)
{
	...
	oz_writeByte(tmp, 8, cd->color.bg_color);
	oz_writeByte(tmp, 16, cd->color.fg_color);
...
oz_CharData oz_deserializeCharData(oz_BitBuffer buffer)
{
	...
	cd.color.bg_color = oz_readByte(&buffer.data[1], 8);
	cd.color.fg_color = oz_readByte(&buffer.data[1], 16);
