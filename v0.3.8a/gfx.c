...
void oz_drawCharData(CHAR_INFO* buffer, oz_CharData c, int x, int y)
{
	...
	buffer[xy].Attributes = (!c.color.transparent_bg ? c.color.bg_color | c.color.fg_color : c.color.fg_color | oz_getWorldAt(x, y).color.bg_color);
}
...
void _print_version(CHAR_INFO* buffer)
{
	if (version == NULL)
	{
		...
		*version = oz_buildString("OmniaZERO - v0.3.8a", DefaultColors.GREEN_ON_BLACK);
	}
	oz_CharData* cd = NULL;
